%Modeling_VSP_tubewave_Elastic_Layer
% S. Minato
...
%==========================================
n=2; % Total number of layers
Vpvec=[4000 3000]; % size n. P-wave velocity at each layer (m/s)
Vsvec=[2000 1000]; % size n. S-wave velocity at each layer (m/s)
Rhovec=[2500 2300]; %Bulk density at each layer (kg/m3)
zn_org=[0]; % size n-1. The depth of boundaries (m). 
...
%--------------------------------------
zvec_rec=[-20.05:0.1:20.05]; %Receiver depth locations (m)
...
irec_H2=find(abs(zvec_rec-10.05)<0.001);

%Analytical solutions
%Direct wave pressure amplitude
tmp=Rhof*CT0vec.^2./mu_vec.*(1-2*Vsvec.^2./Vpvec.^2)./(CT0vec.^2./Vpvec.^2-1); %assuming unit tzz
...
%---Elastic R/T Coefficients--
%Normal incidence
%---displacement formulation
Rpp=(Vp1*Rho1-Vp0*Rho0)/(Vp1*Rho1+Vp0*Rho0); %down-up
Tpp=2*Vp0*Rho0/(Vp1*Rho1+Vp0*Rho0); %down-down
%---potential formulation
R=(Rho1/Vp0-Rho0/Vp1)/(Rho1/Vp0+Rho0/Vp1); %down-up
T=2*Rho0/Vp0/(Rho1/Vp0+Rho0/Vp1); %down-down


VT0=sqrt(Vf^2/(1+Rhof*Vf^2/(Rho0*Vs0^2)));
VT1=sqrt(Vf^2/(1+Rhof*Vf^2/(Rho1*Vs1^2)));

iw=100;%dummy
w=wvec_proc(iw);
kp0=wvec_proc(iw)/Vp0;
kp1=wvec_proc(iw)/Vp1;
k0=wvec_proc(iw)/VT0;
k1=wvec_proc(iw)/VT1;

AP0=wvec_proc(iw).^2./kp0*(1/(2*Vs0^2)-1/Vp0^2);
AP1=wvec_proc(iw).^2./kp1*(1/(2*Vs1^2)-1/Vp1^2);

%top layer
De0=1/(-Rhovec(1)*w^2); %unit amplitude in tzz
p_direct0=2*w^2*Rhof*kp0*AP0*De0/(k0^2-kp0^2);
p_reflect0=R*p_direct0;

p_gen0=Rhof*VT0*w*2*k0*kp0/(kp0^2-k0^2)*AP0*(...
    (k1+kp0)/(k0+k1)*R*De0+(k1-kp0)/(k0+k1)*De0)...
       +Rhof*VT1*w*2*k1*kp1/(k0+k1)/(k1+kp1)*AP1*T*De0;


%bottom layer
p_direct1=-Rhof*VT1*w*2*k1*kp1/(kp1^2-k1^2)*AP1*T*De0;
p_gen1=Rhof*VT1*w*2*k1*kp1/(kp1^2-k1^2)*AP1*(...
    (k0+kp1)/(k0+k1)*T*De0)...
       +Rhof*VT0*w*2*k0*kp0/(k0+k1)*AP0*(...
           R/(k0+kp0)+1/(k0-kp0))*De0;
...
hold on;plot([0.008 0.009],[1 1]*p_reflect0,'k-','linewidth',2)
hold on;plot([0.013 0.014],[1 1]*p_gen0,'k-','linewidth',2)
xlabel('Lapse time (s)')
ylabel('Pressure (Pa)')
title('-10.05m')
tmpa=axis;axis([0 0.02 -0.11 0.05])
grid on;
subplot(212),plot(tvec+delay_Haskel,data_B(:,irec_H2))
hold on;plot([0.0085 0.01],[1 1]*p_direct1,'k-','linewidth',2)
hold on;plot([0.015 0.016],[1 1]*p_gen1,'k-','linewidth',2)
xlabel('Lapse time (s)')
ylabel('Pressure (Pa)')
title('+10.05m')
tmpa=axis;axis([0 0.02 -0.4 0.2])
grid on;
